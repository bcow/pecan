...
define_benchmark <- function(bm.settings, dbcon, user_id = ""){
  
  if (is.MultiSettings(bm.settings)) {
    return(papply(bm.settings, function(x) define_benchmark(x, dbcon)))
  }
  ...
  if(is.null(bm.settings$reference_run_id)){
    if(!is.null(bm.settings$ensemble_id) & length(bm.settings$ensemble_id) > 0){
      
      # check if there is already a BRR for ensemble.id, otherwise make one
      bm_ens <- tbl(dbcon,"benchmarks_ensembles") %>% rename(bm_ensemble_id = id) %>% 
        filter(ensemble_id == !!bm.settings$ensemble_id) %>% collect()
      
      if(nrow(bm_ens) == 0){
        # Get workflow id from ensemble id
        ens_wf <- tbl(dbcon, 'ensembles') %>% filter(id == !!bm.settings$ensemble_id) %>% 
          rename(ensemble_id = id) %>% 
          left_join(.,tbl(dbcon, "workflows") %>% rename(workflow_id = id), by="workflow_id") %>% collect()
        BRR <- create_BRR(ens_wf, dbcon, user_id)
      }else if(nrow(bm_ens) == 1){
        BRR <- tbl(dbcon,"reference_runs") %>% filter(id == !!bm_ens$reference_run_id) %>% 
          rename(reference_run_id = id) %>% collect()
      }else if(nrow(bm_ens) > 1){ # There shouldn't be more than one reference run per run
      ...
      bm.settings$reference_run_id <- as.numeric(BRR$reference_run_id)
  ...
  for(i in which.bm){
    ...
    if(!is.null(benchmark$metrics)){
      ...
      metric_ids <- tbl(dbcon, 'metrics') %>% pull(id)
    }
    ...
    bm <- tbl(dbcon, 'benchmarks') %>% 
      filter(input_id == !!benchmark$input_id) %>%
      filter(variable_id == !!benchmark$variable_id) %>%
      filter(site_id == site_id) %>% collect() %>%  glimpse()
    
    # There is probably a more elegant way to convert all the integer64 values
    if(nrow(bm) > 0){
      bm <- bm %>% 
        hablar::convert(int(id, input_id, variable_id, site_id, user_id))
      }
    ...
    for(k in seq_along(metric_ids)){
      bmmetric <- tbl(dbcon, 'benchmarks_metrics') %>% 
        filter(benchmark_id == !!bm$id) %>%
