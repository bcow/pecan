...
match_species_id <- function(input_codes, format_name = 'custom', bety = NULL, translation_table = NULL, ...) {
  ...
  if (!is.null(translation_table)) {
    ...
    if(!is.null(bety)){
      ...
      translation <- dplyr::semi_join(translation, as.data.frame(input_codes, stringsAsFactors = FALSE), by = "input_codes" )  #Keep rows in translation table that have the same 'latin_name' within obs
    }else{
      # use traits package
